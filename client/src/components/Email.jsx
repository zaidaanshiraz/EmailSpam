    ...
    return (
        <Box style={openDrawer ? { marginLeft: 250, width: 'calc(100% - 250px)' } : { width: '100%' }}>
            <Box style={{ padding: '20px 10px 0 10px', display: 'flex', alignItems: 'center' }}>
                ...
                <DeleteOutline onClick={(e) => deleteEmails(e)} />
            </Box>
            <List>
                {
                    getEmailService?.response?.map(email => (
                        <Box key={email._id}>
                            ...
                            />
                            {/* Show IP address below each email */}
                            {email.ipAddress && (
                                <Typography style={{ fontSize: "12px", marginLeft: "45px", color: email.isSpam ? "red" : "green" }}>
                                    IP: {email.ipAddress} — {email.isSpam ? "⚠ Suspicious" : "✓ Safe"}
                                </Typography>
                            )}
                        </Box>
                    ))
